...
parser.add_argument('--replace_ln_with_tanh', type=bool, default=False, help='Replace LayerNorm with Dynamic tanh, default is False')

parser.add_argument('--max_iters', type=int, default=500, help='Number of iterations to train, default is 5000')
parser.add_argument('--eval_interval', type=int, default=100, help='How many number of iterations between evaluation, default is 100')
parser.add_argument('--learning_rate', type=float, default=3e-4, help='Learning rate for training, default is 3e-4')
parser.add_argument('--eval_iters', type=int, default=200, help='Number of iterations to average loss during evaluation, default is 200')
parser.add_argument('--n_embd', type=int, default=384, help='Embedding dimension, default is 384')      
parser.add_argument('--n_head', type=int, default=6, help='Number of heads, default is 6')
parser.add_argument('--n_layer', type=int, default=6,  help='Number of layers, default is 6')   
...
class DyT(nn.Module):   
    def __init__(self, num_features, alpha_init_value=0.5, *args, **kwargs):
...
class Block(nn.Module):
    ...
    def __init__(self, n_embd, n_head, replace_ln_with_tanh=False):
        # n_embd: embedding dimension, n_head: the number of heads we'd like
        super().__init__()
        head_size = n_embd // n_head
        self.sa = MultiHeadAttention(n_head, head_size)
        self.ffwd = FeedFoward(n_embd)
        if replace_ln_with_tanh:
            self.ln1 = DyT(n_embd)
            self.ln2 = DyT(n_embd)
        else:
...
class GPTLanguageModel(nn.Module):
    ...
    def __init__(self):
        ...
        self.blocks = nn.Sequential(*[Block(n_embd, n_head=n_head, replace_ln_with_tanh=args.replace_ln_with_tanh) for _ in range(n_layer)])
...
if args.replace_ln_with_tanh:
    print("===Replace LayerNorm with tanh===")
else:
    print("===Use LayerNorm===")
...
print(generated)
print(f"====Seconds used for inference: {time_end_inference-time_start_inference}====")
# topen('more.txt', 'w').write(decode(m.generate(context, max_new_tokens=10000)[0].tolist()))